...
Django is a popular open-source "batteries-included" python web framework that enables rapid development while taking out much of the hassle from routine web development. It provides pre-built components like web servers, database ORMs, and authentication/authorization layers, allowing developers to focus on application business logic and reduces prevents common security mistakes. As such, developers and organizations worldwide use Django to build mission-critical applications.

A Django application is composed of multiple interconnected components- HTTP servers, WSGI/ASGI gateways, databases, caches, background workers and more- and issues can be encountered in any component at various stages of application's lifespan. To observe a Django application (including monitoring its vitals for performance concerns), you need to observe all these components. And that’s where OpenTelemetry comes into the picture.

In this article, we will walk you through the relevance of OpenTelemetry for Django applications, instrumenting a sample application with OpenTelemetry, and gain understand how OpenTelemetry-native backends like SigNoz represent the data.

## What is OpenTelemetry Django?
[OpenTelemetry Django](https://signoz.io/docs/instrumentation/django/) instrumentation enables generation and management of telemetry data (logs, metrics, and traces) from your Django application. This data is then used to observe the application and gain insights into its behaviour, performance and health. OpenTelemetry provides an open-source standard with a consistent collection mechanism and data format. As application owners, you will always have the freedom to choose different vendors to visualize the collected telemetry data, and can export this data in a variety of formats.
...
Let's go through the steps to setting up and instrumenting a Django application with OpenTelemetry to automatically emit traces and metrics. This concept is known as automatic instrumentation as there are no modifications to the codebase.
We'll be exporting the emitted telemetry to SigNoz.
...
We have prepared a [sample Django application](https://github.com/SigNoz/sample-django) to speed up the setup process. We'll be using Django's admin panel to create and interact with polls.
...
Django applications require some setup before they can be run- operations like database migrations, superuser creation and so on, and the same is true here.
...
After running the command, visit [http://localhost:8000/admin](http://localhost:8000/admin) to open the admin panel and validate your app is running. Generate some telemetry to be captured by adding few questions through the app interface.
...
As previously discussed, to send the collected data to SigNoz, you need to set some environment variables while running the application with OpenTelemetry. The following command will set requisite values and start the container:
...
<figure data-zoomable align='center'>
    <img src="/img/blog/2025/11/otel-django-logs.webp" alt="Application Logs in SigNoz Views Pane"/>
    <figcaption><i>Application Logs in SigNoz Views Pane</i></figcaption>
</figure>

## Next Steps

TODO: research and add points here

add logs export segment to django logs article and links here

other sections link from otel python blog
