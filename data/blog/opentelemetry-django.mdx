...
A Django application is composed of multiple interconnected components — HTTP servers, WSGI/ASGI gateways, databases, caches, background workers and more — and components may misbehave at various stages of the application's lifespan. To observe a Django application (including monitoring its vitals for performance concerns), you need to observe all these components. And that’s where OpenTelemetry comes into the picture.

In this article, we will walk you through the relevance of OpenTelemetry for Django applications, instrumenting a sample application with OpenTelemetry, and understand how OpenTelemetry-native platforms like SigNoz utilize telemetry.

## What is OpenTelemetry Django?

[OpenTelemetry Django](https://signoz.io/docs/instrumentation/django/) instrumentation enables generation and management of telemetry data (logs, metrics, and traces) from your Django application. This data is then used to observe the application and gain insights into its performance and behavior. OpenTelemetry provides an open-source standard with a consistent data format and collection mechanism. As application owners, you will always have the freedom to choose different vendors to visualize the collected telemetry data, and can consume this data in a variety of formats.
...
Observability is a practice that should evolve as your applications do, and part of that is manually instrumenting parts of the codebase that are otherwise unobserved. Having custom traces and metrics will help you capture details that matter to your application systems, help deliver business value and recognize and resolve complex issues faster.
