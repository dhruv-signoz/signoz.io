...
By instrumenting your application code with Redis telemetry libraries, you will capture the queries generated by the Redis engine. Combined with rest of the application flow, these will provide you insight into how Redis performs on a per-request basis.
Consider this, do the abnormally long cache `GET` operations cause high latencies for the `user-info` endpoint?

OpenTelemetry supports Redis instrumentation for [all major languages](https://opentelemetry.io/ecosystem/registry/?s=redis&flag=all).
