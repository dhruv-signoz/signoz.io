...
{/* <!-- TODO: update all images using growth.signoz.io backend --> */}
...
2. **MongoDB**  
    ...
    On macOS the installation is done using Homebrew's brew package manager. Once the installation is done, don't forget to start MongoDB services using `brew services start mongodb/brew/mongodb-community@x.x` on your macOS terminal (replace x.x with the MongoDB version).
